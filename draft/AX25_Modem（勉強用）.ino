...
#define FLAG 0x7E // dup : AX25_FLAG at ax25.h

//Protocol Identifier. 0xF0 means : No Layer 3 protocol implemented
#define PID 0xF0 // dup : AX25_PROTOCOL at ax25.h

//Control Field Type for Unnumbered Information Frames : 0x03
#define CONTROL 0x03 // dup : AX25_CONTROL at ax25.h

#define MAX_LENGTH 280 // dup : MAX_LENGTH at ax25.h
#define MAX_LENGTH_FINAL 450 // dup : MAX_LENGTH_FINAL at ax25.h
//CRC-CCITT
#define CRC_POLYGEN     0x1021 // dup : CRC_POLYGEN

char SrcCallsign[7] = "000000"; // 送信元コールサイン // dup : setFromCallsign()の引数 at ax25.cpp
char DestCallsign[7] = "000000"; // 送信先コールサイン // dup : setToCallsign()の引数 at ax25.cpp

byte ssid_source = 0; // 変数をセットする必要はなさそう // dup : AX25_SSID_SOURCE at ax25.h
byte ssid_destination = 0; // 変数をセットする必要はなさそう // dup : AX25_SSID_DESTINATION at ax25.h

byte bitSequence[280*8]; // dup bitSequence at ax25.h
byte finalSequence[450]; // dup finalSequence at ax25.h
byte RcvSequence[450]; // dup receive()の第一引数 at ax25.cpp
...
/*
  ...
  【概要】　文字列をコピーする関数。終端文字 '\0' も含めてコピーする。
  【構文】　strcpy(char *dest, const char *src);
  【引数】
    dest : コピー先（書き込み可能な文字配列またはポインタ）
    src  : コピー元（定数文字列または文字配列）
  【戻り値】
    コピー先（dest）のポインタを返す。
  【注意】
    ・コピー先の領域はコピー元の文字列長 + 1 以上必要。
    ・サイズ不足だとバッファオーバーフローの危険あり。
*/
...
// MEMO : inline を前につけることでインライン関数となる、インライン関数は引数を渡す際のメモリ移動を無くすために、コンパイル時に呼び出し場所に内容が展開される
...
// MEMO : Arduino言語ではコンパイル時にプロトタイプ宣言を自動挿入してるらしい
...
{  
  ...
  //Add Message
  for (int i=0; i < strlen(message) ; i++) bitSequence[Index++] = message[i]; // MEMO : string.h が必要
  ...
  //Add FCS in MSB form
  //Add MS byte
  bitSequence[Index++] = (FCS >> 8) & 0xff; // CRC の上位8bit取り出し
  //Add LS byte
  bitSequence[Index++] = FCS & 0xff; // CRC の下位8bit取り出し
...
void AddHeader(byte *Buffer) // dup : AddHeader() at ax25.cpp
{

    //Shift bits 1 place to the left in order to allow for HDLC extension bit
    for (int i=0; i < strlen(DestCallsign) ; i++) Buffer[Index++] = DestCallsign[i]<<1; // MEMO : Buffer[index] に格納後、indexを+1
...
void BitProcessing(byte *Buffer, uint8_t bytelength) // dup : bitProcessing() at ax25.cpp
{
...
void Demod(byte *Buffer, uint8_t bytelength) // dup : demod() at ax25.cpp
{
...
boolean logicXOR(boolean a, boolean b) // dup : logicXOR() at AX25.cpp
{
  return (a||b) && !(a && b); 
}

unsigned int CRC_CCITT (byte *Buffer, uint8_t bytelength) // dup : crcCcitt at ax25.cpp
{
  uint8_t OutBit = 0;
